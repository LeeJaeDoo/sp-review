 ...
 */
@Component
class ReviewErrorAttributes : ErrorAttributes {
    override fun getError(request: ServerRequest): Throwable {
        return request.attributeOrNull("REVIEW") as Throwable?
    ...
    private fun determineHttpStatus(error: Throwable): HttpStatus = when (error) {
        ...
        is ReviewFrontException -> AnnotatedElementUtils.findMergedAnnotation(
            error::class.java,
    ...
    private fun determineCode(error: Throwable): String = when (error) {
        ...
        is ReviewFrontException -> error.errorCode.simpleCode
    ...
    private fun determineMessage(error: Throwable, internalApi: Boolean): String = when (error) {
        ...
        is ReviewFrontException -> error.message
